...
export default function Stories({ stories, filter, openCollectiveModal }) {
  ...
  const show = currentStories?.length;

  return (
    <AnimateHeight id="stories" height={show ? 'auto' : 0} duration={300}>
      <div className="mb-12 px-4 lg:px-0">
        <div
          className={`mb-6 flex items-center justify-between transition-opacity ${show ? 'opacity-100' : 'opacity-0'}`}
        >
